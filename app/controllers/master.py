...
class MasterController(BaseController):
    ...
    def register_routes(self):
        ...
        self.app.add_url_rule('/master/team-performance-excel/<int:team_id>', 'master.team_performance_excel',
                             self.master_required(self.team_performance_excel))
    ...
    def _create_team_performance_excel(self, team, month, company):
        ...
        for member in team_members:
            ...
            submitted_plus_fees = submission_metrics['total_submitted']
            ...
            # print(type(member))
            # goal = member.get_yearly_goal(company)
            # for goal in member.get_yearly_goal:
            # print("Goal: ", goal)
            
            target_activity = 250
            target_submitted = member.get_yearly_goal(company)/12
        ...
        for row, member in enumerate(member_data, 8):
            for col, value in enumerate(member, 1):
                ...
                if col == 11 or col == 13:  # Submitted amounts
                    if isinstance(value, (int, float)):
                        cell.number_format = '"£"#,##0.00'
                
                # Center align certain columns
                if col in [2, 3, 4, 5, 6, 7, 8, 9, 10, 12, 13, 14]:
        ...
        for col_idx, col in enumerate(ws.iter_cols(min_col=1, max_col=ws.max_column), start=1):
            ...
            for cell in col:
                ...
                # Handle multi-line strings; fall back to str() for numbers/dates
                text = str(cell.value)
                max_len = max(max_len, max(len(line) for line in text.splitlines() or [""]))
